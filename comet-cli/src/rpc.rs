...
pub fn generate_rust_client_with_types(manifest: &RpcManifest, types: &TsTypes) -> String {
    let json_routes = manifest
        .routes
        .iter()
        .filter(|route| route.support == RpcRouteSupport::Json)
        .collect::<Vec<_>>();
    let declarations = rust_type_declarations(&json_routes, types);
    let methods = json_routes
        .iter()
        .map(|route| render_rust_method(route))
        .collect::<Vec<_>>()
        .join("\n\n");
    ...
    format!(
        "// Generated by comet rpc generate. Do not edit by hand.\n\n\
         ...
         \x20   }}\n\n\
         \x20   pub fn with_bearer_token(mut self, token: impl Into<String>) -> Self {{\n\
         ...
         \x20   }}\n\n\
         {methods}\n\n\
         \x20   async fn request<T: serde::de::DeserializeOwned, B: Serialize + ?Sized>(\n\
         \x20       &self,\n\
         \x20       method: reqwest::Method,\n\
         \x20       path: String,\n\
         \x20       body: Option<&B>,\n\
         \x20       auth: bool,\n\
         \x20   ) -> Result<T, CometRpcError> {{\n\
...
fn rust_type_declarations(routes: &[&RpcRoute], types: &TsTypes) -> Vec<String> {
    let mut names = typescript_type_names(routes);
    names.extend(types.models.keys().cloned());
    names.extend(types.enums.keys().cloned());
    names
        ...
        .map(|name| {
            if let Some(model) = types.models.get(&name) {
                render_rust_struct(model)
            } else if let Some(ts_enum) = types.enums.get(&name) {
                render_rust_enum(ts_enum)
            } else {
...
fn render_rust_struct(model: &TsModel) -> String {
    let fields = model
        .fields
        .iter()
        .map(|field| {
            let ident = sanitize_rust_identifier(&field.name);
            let rename = if ident == field.name {
                String::new()
            } else {
                format!("    #[serde(rename = {:?})]\n", field.name)
            };
            format!(
                "{rename}    pub {ident}: {},",
                client_rust_type(&field.rust_type)
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
...
fn render_rust_enum(ts_enum: &TsEnum) -> String {
    let variants = ts_enum
        .variants
        .iter()
        .map(|variant| {
            let rust_name = to_pascal_case(variant);
            format!("    #[serde(rename = {:?})]\n    {rust_name},", variant)
        })
        .collect::<Vec<_>>()
        .join("\n");
    ...
    format!(
        ...
        ts_enum.name, variants
    )
}

fn render_rust_method(route: &RpcRoute) -> String {
    let method_name = route.name.clone();
    let return_type = route
        .response
        .as_deref()
        .map(client_rust_type)
    ...
    let mut params = route
        ...
        .map(|param| {
            let rust_type = if param.variadic {
                ...
                client_rust_type(&param.rust_type)
            };
            format!("{}: {}", sanitize_rust_identifier(&param.name), rust_type)
        })
        .collect::<Vec<_>>();
    ...
    if let Some(body) = &route.body {
        params.push(format!(
            "{}: &{}",
            sanitize_rust_identifier(body_arg),
            client_rust_type(body)
        ));
    }

    let path_expr = rust_path_expr(route);
    let body_expr = if route.body.is_some() {
        format!("Some({})", sanitize_rust_identifier(body_arg))
    } else {
    ...
    format!(
        "    pub async fn {method_name}({signature_params}) -> Result<{return_type}, CometRpcError> {{\n\
...
fn rust_path_expr(route: &RpcRoute) -> String {
    ...
    let mut format_string = route.path.clone();
    ...
    for param in &route.path_params {
        ...
        if param.variadic {
            args.push(format!(
                "{ident}.into_iter().map(encode_path_value).collect::<Vec<_>>().join(\"/\")"
            ));
        } else {
...
fn client_rust_type(rust_type: &str) -> String {
    let rust_type = rust_type.trim();
    if let Some(inner) = generic_inner_for_last_segment(rust_type, "Option") {
        return format!("Option<{}>", client_rust_type(&inner));
    }
    if let Some(inner) = generic_inner_for_last_segment(rust_type, "Vec") {
        return format!("Vec<{}>", client_rust_type(&inner));
    }
...
fn sanitize_rust_identifier(value: &str) -> String {
    ...
    match ident.as_str() {
        "" => "value".to_owned(),
        "type" | "match" | "ref" | "self" | "Self" | "crate" | "super" | "enum" | "struct"
        | "fn" | "async" | "await" | "impl" | "trait" | "where" | "move" | "loop" | "for"
        | "while" | "use" | "mod" | "pub" | "const" | "static" | "let" => format!("r#{ident}"),
        _ => ident,
    }
}
...
mod tests {
    ...
    #[test]
    fn generates_rust_client_for_json_routes() {
        let manifest = RpcManifest {
            version: MANIFEST_VERSION,
            routes: vec![RpcRoute {
                name: "create_task".to_owned(),
        ...
        assert!(rust.contains("pub struct NewTask"));
        assert!(rust.contains("pub title: String,"));
        assert!(rust.contains("pub enum TaskStatus"));
        assert!(rust.contains("#[serde(rename = \"in_progress\")]"));
