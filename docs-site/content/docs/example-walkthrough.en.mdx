---
title: Full example
description: A walkthrough of examples/cloudflare-worker — D1, Queues, R2, WebSocket, and auth together.
---
...
WebSocket echo endpoint, `comet-auth` mounted in the same Worker, and
end-to-end unit + integration tests. This page walks through the main pieces.
...
examples/cloudflare-worker/
  src/
    ...
    app.rs           # builds the Rocket, body limits, auth, .manage(env)
...
| `GET /auth/<provider>/start` | starts Google, Apple, or GitHub OAuth |
| `POST /auth/native/google`, `POST /auth/native/apple` | native login by identity token |
| `POST /auth/logout` | revokes the current session |
| `GET /private/me` | protected route with `#[comet_auth::requires_auth]` |
| `GET /private/admin` | RBAC route with `#[comet_auth::requires_auth(role = "admin")]` |
| `GET /private/reviewer` | RBAC route with `any(...)` and `resource = "demo"` |
...
## Auth: session and protected route

`src/app.rs` mounts auth routes in the same Rocket:
...
And `src/demo/routes.rs` shows the expected ergonomics for protected handlers:
...
Public routes stay public. Without a session, `/private/me`,
`/private/admin`, and `/private/reviewer` return `401 Unauthorized`, while `/auth/session` returns
`authenticated: false`. With a valid session but without the required role or
permission, an RBAC route returns `403 Forbidden`.
...
The binding names (`DB`, `AUTH_KV`, `ASSETS`, `TASK_EVENTS`) need to match the
...
# paste the database_id and KV id into wrangler.jsonc
