...
const copy = {
  'pt-BR': {
    tagline: (
      <>
        Rode rotas <span className="font-medium text-fd-foreground">Rocket</span> em{' '}
        <span className="font-medium text-fd-foreground">Cloudflare Workers</span>, com um fork
        remendado do Rocket que compila para <code>wasm32-unknown-unknown</code> — com um ORM
        D1-first opcional e uma CLI para gerar tudo isso.
      </>
    ),
    ...
    features: [
      {
        title: 'Rocket em Workers',
        description:
          'Escreva handlers de rota comuns do Rocket — guards, responders, fairings — e rode-os sem modificação em Cloudflare Workers, via dispatch direto em um núcleo Rocket remendado. Sem sockets, sem Hyper.',
        href: '/pt-BR/docs/cloudflare-adapter',
      },
      {
        title: 'Nebula ORM',
        description:
          'Um núcleo de ORM D1-first: #[derive(Entity)], colunas tipadas, builders de SQL determinísticos, relacionamentos explícitos e geração segura de migrations — totalmente opcional.',
    ...
    features: [
      {
        ...
        href: '/en/docs/cloudflare-adapter',
      },
      {
        title: 'Nebula ORM',
        description:
...
export default async function HomePage({ params }: { params: Promise<Params> }) {
  ...
  const t = lang === 'pt-BR' ? copy['pt-BR'] : copy.en;
  ...
  return (
    <main className="flex flex-1 flex-col">
      <section className="flex flex-col items-center gap-6 border-b border-fd-border bg-fd-secondary/30 px-6 py-24 text-center">
        ...
        <p className="max-w-2xl text-fd-muted-foreground sm:text-lg">{t.tagline}</p>
