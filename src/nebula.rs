...
/// Owned, serializable mirrors of the `'static`-only schema metadata types.
///
/// `TableDef`/`ColumnDef`/`IndexDef`/`ForeignKeyDef` are built from
/// `&'static str` by design: Nebula never reflects on runtime data, and every
/// identifier that reaches generated SQL comes from a Rust compile-time
/// constant. Tooling that needs to persist a schema snapshot to disk (to
/// diff "what's on disk from the last migration" against "what the code
/// currently declares") can't get `'static` data back out of a file, though.
///
/// Rather than duplicate the diff/format engine above for an owned type
/// family, this module converts owned data to `'static` by leaking it
/// (`Box::leak`), which is safe and idiomatic for a short-lived, one-shot
/// process such as a CLI invocation, and then hands off to the existing,
/// already-tested `SchemaManifest` APIs unchanged.
